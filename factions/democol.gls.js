return (fm) => {
	...
	fm.add('DEMOCOLLECTIVE', {
		...
		colors: { // here we define colors for faction-specific things such as text
			faction: #to_color(250, 50, 250), // RGB, bytes, 0 to 255
			faction_shadow: #to_color(125, 20, 125),
			text: #to_color(1.0, 0.6, 1.0), // RGB, floats, 0.0 to 1.0
			text_shadow: #to_color(1.0, 0.6, 1.0, 0.5), // RGBA, floats (last one is alpha channel, 0.5 means 50% transparent, 1.0 means non-transparent)
			border: #to_color(250, 120, 250, 200), // RGBA, bytes
		...
		bases: {
			render: { // here we are telling GLSMAC where to find sprites for bases
				type: 'sprite_grid', // base sprites are aligned in 4 columns, 6 rows, same as in original faction PCX files
				...
				cell_width: 400, cell_height: 300, // define size of each grid element (each base sprite). note that our png is 4x larger than PCX of factions in original SMAC
